...
import { updateGroupAction, fetchGroups } from '../../store/group';
...
function UpdateGroupForm() {
    ...
    useEffect(() => {
        if (group) {
            let conjoinedLocation = group.city + ', ' + group.state;
            setLocation(conjoinedLocation ? conjoinedLocation : '');
            setName(group.name ? group.name : '');
            setAbout(group.about ? group.about : '');
            setType(group.type ? group.type : '');
            setPrivacy(group.private ? group.private : false);
        }
    }, [group]);
